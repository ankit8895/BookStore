...
const BooksByGenre = () => {
  ...
  const updatedGenre = name.split('-').join(' ');
  const dispatch = useDispatch();
  const [isOpen, setIsOpen] = useState({});

  const { loading, error, books } = useSelector(
    (state) => state.allGenreBooksList
  );

  useEffect(() => {
    if (books.length === 0) {
      dispatch(fetchAllBooksByGenre(updatedGenre));
  ...
  useEffect(() => {
    return () => {
      dispatch(allGenreBooksAction.resetGenreBooks());
    };
  }, []);
  ...
  return (
    <Section classname='flex flex-col items-start tracking-widest'>
      <div className='container max-w-5xl mx-auto text-center mt-4 md:mt-16'>
        ...
        <p className='text-lg text-white mb-8'>
          All the books of genre {updatedGenre}
        </p>
      </div>
              ...
              {isOpen[book.id] && (
                ...
                >
                  <p className='text-justify mb-2'>
                    <span className='text-cyan-500'>Buy Now: </span>
                    <Link
